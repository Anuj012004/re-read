...
//               <TabsTrigger value="books" className="flex items-center gap-2 data-[state=active]:bg-orange-400 cursor-pointer
...
//                 Books
//               </TabsTrigger>
//               <TabsTrigger value="organizations" className="flex items-center gap-2 data-[state=active]:bg-orange-400 cursor-pointer
// data-[state=active]:text-black data-[state=active]:shadow 
//           bg-gray-200 
//           text-gray-700 
//           rounded-lg 
...
//                 <Button onClick={fetchBooks} variant="outline" className="cursor-pointer">Try Again</Button>
//               </div>
//             ) : books.length === 0 ? (
//               <div className="text-center py-20">
...
//                   {!selectedOrg && (
//                     <Button variant="outline" size="lg" onClick={handleBrowseAll} className="cursor-pointer bg-orange-100">
//                       Browse All Books
//                     </Button>
//                   )}
//                 </div>
...
export const BooksShowcase = ({ isLoggedIn, onLogin }: BooksShowcaseProps) => {
  ...
  const fetchBooks = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await api.get("/api/books", { withCredentials: true });
      const fetchedBooks = res.data.books || [];
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      // Update allBooks state and recalculate organizations
      setAllBooks((prev) => {
        const updated = prev.filter((b) => b._id !== deleteBookId);
        calculateOrganizations(updated);
        return updated;
      });
